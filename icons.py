...
# with a maximized / full-screen window on top it goes by that window's title
# bar (a small strip at the top right is sampled), otherwise by the wallpaper
# as a whole (a dark sky over a light landscape still counts as light).
...
def maximized_window_on_top() -> bool:
    """True when a maximized or full-screen window fills the top of the screen
    (then MyDockFinder colours its bar by that window's title bar); False when
    the desktop shows through (then it goes by the wallpaper)."""
    if sys.platform != "win32":
        return False
    ctypes, wintypes, user32, gdi32 = _win32()
    sw, sh = _screen_size()
    for px, py in ((sw // 2, 60), (sw // 2, sh // 2)):
        hwnd = user32.WindowFromPoint(wintypes.POINT(px, py))
        if not hwnd:
            continue
        root = user32.GetAncestor(hwnd, 2) or hwnd                          # GA_ROOT
        cls = ctypes.create_unicode_buffer(64)
        user32.GetClassNameW(root, cls, 64)
        if cls.value in ("Progman", "WorkerW"):                             # the desktop
            continue
        if user32.IsZoomed(root):
            return True
        r = wintypes.RECT()
        if user32.GetWindowRect(root, ctypes.byref(r)) and r.left <= 0 and r.top <= 0 \
                and r.right >= sw and r.bottom >= sh:
...
def shell_bar_is_light(prev: Optional[bool]) -> Optional[bool]:
    """Mirror of how MyDockFinder picks its bar colour: by the title bar of a
    maximized / full-screen window, otherwise by the wallpaper as a whole."""
    try:
        if maximized_window_on_top():
            img = grab_top_bar()
...
def theme_report() -> list:
    ...
    try:
        lines.append(f"maximized / full-screen window on top: {maximized_window_on_top()}")
