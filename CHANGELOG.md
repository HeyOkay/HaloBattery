...
- Logitech support over HID++ 2.0, without G HUB (and alongside it). Every device
  ...
  matches G HUB) and the G502 X PLUS (unified battery); other HID++ 2.0 mice and
  keyboards should work the same way.
...
- After a device went missing (e.g. a Razer headset switched off while its receiver
  ...
  ran, instead of at the poll interval: the quick re-check that confirms a disconnect never
  stopped after the icon had been removed. Besides filling the log, this would keep
  waking wireless mice (e.g. Logitech) over the radio.
